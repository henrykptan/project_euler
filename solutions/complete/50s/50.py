...
# 3i. AND once our sum for a certain starting number exceeds 100 we can stop searching /
# 4. Don't need to keep searching if current longest sequence is longer than the number of primes left to search
# 5. IMPORTANT search from longest to shortest, as then when prime sum less than the limit is found we can stop for
#   a given starting number n, KEEPING TRACK OF THE END NUMBER m
        ...
        for end_index in range(max_index + 1, len(longest_seq), -1):  # (note final_index +1 by 5.) start from the end
            ...
            if total > limit:  # if our running sum is greater than the limit
                ...
                if len(sequence) > len(longest_seq):
                    # and if the sequence is longer than out current longest we replace
...
if __name__ == '__main__':
    ...
    print("sum = {}, longest sequence = {}, sequence length = {}, time taken = {}".format(sum(ans), ans, len(ans),
                                                                                          time.time() - start))
